...
export class NordstromTopPickGuidesElement {
    ...
    public async getActiveGuideName(): Promise<string> {
        const allGuides = await this.baseLocator.locator('nav>ul>li.ZgHf6').all();

        for (const guide of allGuides) {
            const classAttr = await guide.getAttribute('class');
            if (classAttr?.includes('CtGs9')) {
                const text = await guide.textContent();
                return text?.trim() ?? '';
            }
        }
        throw new Error('Active guide with class CtGs9 not found');
    }
}
